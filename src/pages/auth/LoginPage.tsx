...
import { OTPModal } from '../../components/ui/OTPModal'; // 1. OTP Modal ko yahan connect kiya
...
export const LoginPage: React.FC = () => {
  ...
  const [showOTP, setShowOTP] = useState(false); // 2. Ye state modal ko kholay gi
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Pehle login credentials check honge
      await login(email, password, role);
      setIsLoading(false);
      
      // 3. AGAR login sahi hai, to dashboard nahi jaye ga balkay OTP dikhaye ga
      setShowOTP(true); 
    } catch (err) {
  ...
  // 4. Jab user sahi OTP code daal dega, tab ye function usay dashboard bhej dega
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      {/* 5. OTP Security Modal - Ye screen par popup laye ga */}
      <OTPModal 
        isOpen={showOTP} 
